...
const { uploadMultiImages } = require("../../utils/fileUpload");
...
async function handleFileUploads(files) {
  ...
  const uploadDir = path.join(
    __dirname,
    "../../public/uploads/trade_responses",
  );
...
class TradeResponseController {
  ...
  static async respondToPost(req, res) {
    try {
      ...
      if (isNaN(giveawayId) || giveawayId <= 0)
        return res
          .status(400)
          .json({ success: false, error: "Invalid giveaway ID" });
      ...
      if (!giveaway)
        return res
          .status(404)
          .json({ success: false, error: "Post not found or not active" });
      ...
      if (existing)
        return res.status(409).json({
          success: false,
          error:
            "You already have a pending offer on this post. Cancel it first.",
      ...
      if (
        !return_type ||
        !["item", "price", "free", "existing"].includes(return_type)
      )
        return res.status(400).json({
          success: false,
          error: "return_type required: item | price | free | existing",
        });
      ...
      if (return_type === "item" && !req.body.item_name)
        return res.status(400).json({
          success: false,
          error: "item_name is required when return_type is item",
        });

      if (return_type === "price" && !req.body.price_range_start)
        return res.status(400).json({
          success: false,
          error: "price_range_start is required when return_type is price",
        });
      ...
      const returnImagePaths = [];

      const MAX_IMAGES = 5;

      let returnImages = req.files?.images;

      if (returnImages) {
        // if single images
        if (!Array.isArray(returnImages)) {
          returnImages = [returnImages];
        }

        // max img upload
        if (returnImages.length > MAX_IMAGES) {
          return res.status(400).json({
            success: false,
        ...
        const uploadImages = await uploadMultiImages(
          returnImages,
          "tradeimages",
        );
        ...
        if (uploadImages.length === 0) {
          return res.status(500).json({
            success: false,
            message: "Image upload failed: " + uploadImages.message,
          });
        }

        for (let img of uploadImages) {
          returnImagePaths.push(img.url);
        }
      }
      ...
      const [result] = await connection.query(
        "INSERT INTO trade_responses SET ?",
        [data],
      );
      ...
      res.status(201).json({
        success: true,
        message:
          "Your offer has been sent! Waiting for the post owner to respond.",
        data: {
          id: result.insertId,
          ...data,
    ...
    try {
      ...
      if (giveaway.user_id !== userId)
        return res.status(403).json({
          success: false,
          error: "You can only view responses on your own posts",
        });
  ...
  static async acceptResponse(req, res) {
    try {
      ...
      if (tradeResponse.poster_user_id !== userId)
        return res.status(403).json({
          success: false,
          error: "Only the post owner can accept offers",
        });

      if (tradeResponse.status !== "pending")
        return res.status(400).json({
          success: false,
          error: `Cannot accept. Current status: ${tradeResponse.status}`,
        });

      const meeting_type = req.body.meeting_type;
      if (
        !meeting_type ||
        !["come_to_me", "i_pick_up", "centre_point"].includes(meeting_type)
      )
        return res.status(400).json({
          success: false,
          error: "meeting_type required: come_to_me | i_pick_up | centre_point",
        });
      ...
      await connection.query(
        ...
        [
          ...
          req.body.meeting_latitude
            ? parseFloat(req.body.meeting_latitude)
            : null,
          req.body.meeting_longitude
            ? parseFloat(req.body.meeting_longitude)
            : null,
      ...
      res.json({
        ...
        data: {
          ...
          giving_item_images: (() => {
            try {
              return JSON.parse(updated.giving_item_images || "[]");
            } catch (e) {
              return [];
            }
          })(),
  ...
  static async confirmPayment(req, res) {
    try {
      ...
      const [[tradeResponse]] = await connection.query(
        `SELECT * FROM trade_responses WHERE id = ?`,
        [responseId],
      );
      if (!tradeResponse)
        return res
          .status(404)
          .json({ success: false, error: "Trade response not found" });
      ...
      if (tradeResponse.status !== "accepted")
        return res.status(400).json({
          success: false,
          error: "Trade must be accepted before confirming payment",
        });

      const payment_amount = parseFloat(
        req.body.payment_amount || tradeResponse.offer_price || "0",
      );
  ...
  static async rejectResponse(req, res) {
    try {
      ...
      if (
        tradeResponse.poster_user_id !== userId &&
        tradeResponse.responder_user_id !== userId
      )
        return res.status(403).json({
          success: false,
          error: "Not authorized to reject this trade",
        });

      if (!["pending", "accepted"].includes(tradeResponse.status))
        return res.status(400).json({
          success: false,
          error: `Cannot reject. Status: ${tradeResponse.status}`,
        });
  ...
  static async cancelResponse(req, res) {
    try {
      ...
      if (tradeResponse.responder_user_id !== userId)
        return res.status(403).json({
          success: false,
          error: "Only the offer sender can cancel it",
        });
  ...
  static async completeTrade(req, res) {
    try {
      ...
      const amount = parseFloat(
        req.body.amount ||
          tradeResponse.payment_amount ||
          tradeResponse.offer_price ||
          tradeResponse.price_range_start ||
          0,
      );
      await connection.query(
        `UPDATE trade_responses SET 
  ...
  static async getMyTrades(req, res) {
    try {
      ...
      if (role === "poster") {
        ...
        whereClause =
          "WHERE (tr.poster_user_id = ? OR tr.responder_user_id = ?)";
  ...
  static async getTradeById(req, res) {
    try {
      ...
      if (!trade)
        return res
          .status(404)
          .json({ success: false, error: "Trade not found or access denied" });
      ...
      res.json({
        ...
        data: {
          ...
          giving_item_images: (() => {
            try {
              return JSON.parse(trade.giving_item_images || "[]");
            } catch (e) {
              return [];
            }
          })(),
          return_item_images: (() => {
            try {
              return JSON.parse(trade.return_item_images || "[]");
            } catch (e) {
              return [];
            }
          })(),
  ...
  static async updateResponseStatus(req, res) {
    try {
      ...
      if (tradeResponse.poster_user_id !== userId)
        return res.status(403).json({
          success: false,
          error: "Only the post owner can accept or reject",
        });

      if (tradeResponse.status !== "pending")
        return res.status(400).json({
          success: false,
          error: `Cannot update. Current status: ${tradeResponse.status}`,
        });
      ...
      if (status === "accepted") {
        ...
        if (meeting_type && !validPreferences.includes(meeting_type))
          return res.status(400).json({
            success: false,
            error:
              "meeting_type must be: Come to me | I Pick Up | Centre Point",
          });
