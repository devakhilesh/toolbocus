...
class AuthController {
    ...
    static async login(req, res) {
        try {
            ...
            // Send OTP
            await connection.query('DELETE FROM otps WHERE phone_number = ?', [phone_number]);
    ...
    static async completeProfile(req, res) {
        try {
            const {
                ...
                gender, location, latitude, longitude, terms_accepted
            } = req.body;

            if (!user_id || !full_name || !email) {
                return res.status(400).json({ success: false, error: 'User ID, full name, and email are required' });
            }

            if (!terms_accepted) {
                return res.status(400).json({ success: false, error: 'You must accept the Terms & Privacy Policy' });
            }
            ...
            const [updateResult] = await connection.query(
                `UPDATE users SET 
                 ...
                 WHERE id = ?`,
                [
                    full_name, email, date_of_birth || null, gender || null,
                    location || null, latitude || 0, longitude || 0,
                    terms_accepted ? 1 : 0, user_id
                ]
            );
            ...
            const [userRows] = await connection.query('SELECT * FROM users WHERE id = ?', [user_id]);
            const user = userRows[0];
   ...
   static async updateProfile(req, res) {
    try {
        ...
        const vis = (profile_visibility !== undefined && profile_visibility !== '')
            ? (profile_visibility == 1 || profile_visibility === 'true' ? 1 : 0)
            : null;
...
static async submitUserReview(req, res) {
    try {
        ...
        if (!reviewer_user_id) return res.status(401).json({ success: false, error: 'Unauthorized' });

        const { user_id, rating, feedback_label, comment } = req.body;

        if (!user_id || !rating) {
            return res.status(400).json({ success: false, error: 'user_id and rating are required' });
        }
        ...
        await connection.query(
            `INSERT INTO reviews (reviewer_user_id, reviewed_user_id, rating, feedback_label, comment, created_at) 
             VALUES (?, ?, ?, ?, ?, NOW())`,
            [reviewer_user_id, user_id, rating, feedback_label || null, comment || null]
        );
