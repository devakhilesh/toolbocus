...
------------------------------ */
      ...
      let itemImages = req.files?.item_images;
      ...
      if (itemImages) {
        ...
        // max img upload
        if (itemImages.length > MAX_IMAGES) {
          return res.status(400).json({
            success: false,
        ...
        const uploadImages = await uploadMultiImages(itemImages, "postimages");
        ...
        for (let img of uploadImages) {
          itemImagePaths.push(img.url);
        }
      }

      // for return item images
      let returnImagePaths = [];

      let returnImages = req.files?.return_item_images;
...
------------------------------ */
      ...
      const [result] = await connection.query("INSERT INTO giveaways SET ?", [
        data,
      ]);
...
------------------------------ */
  ...
  static async getPosts(req, res) {
    try {
      ...
      if (search) {
        conditions.push(
          `(g.item_name LIKE ? OR g.return_item_name LIKE ? OR g.item_note LIKE ?)`,
        );
        const like = `%${search}%`;
...
`,
  ...
  static async getMyPosts(req, res) {
    try {
      ...
      const { type = "all", page = 1, limit = 10 } = req.body;
      ...
      res
        .status(500)
        .json({ success: false, error: "Failed to fetch your posts" });
    }
  }
  ...
  static async updatePost(req, res) {
    try {
      ...
      //     body.item_images = JSON.stringify(
      //       await handleFileUploads(req.files.item_images),
      //     );
      //   }
      //   if (req.files?.return_item_images) {
      //     body.return_item_images = JSON.stringify(
      //       await handleFileUploads(req.files.return_item_images),
      //     );
      //   }
      ...
      if (itemImages) {
        ...
        body.item_images = JSON.stringify(itemImagePaths);
      }

      //====== for return_item_images
      ...
      if (returnImages) {
        ...
        body.return_item_images = JSON.stringify(returnImagePaths);
      }
