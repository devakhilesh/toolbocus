...
exports.save_category = async (req, res) => {
    try {
        ...
        const sql = "INSERT INTO categories (name, sub_id, image_url, status) VALUES (?, ?, ?, ?)";
        await connection.query(sql, [name, sub_id, image_url, status]);
...
exports.update_category = async (req, res) => {
    try {
        ...
        if (req.file) {
            ...
            sql = "UPDATE categories SET name = ?, status = ?, image_url = ? WHERE id = ?";
            params = [name, status, image_url, id];
        } else {
    ...
    try {
        ...
        await connection.query(
            `INSERT INTO onboarding_screens (title, description, image_url, screen_order, status)
             VALUES (?, ?, ?, ?, ?)`,
            [title, description, image_url, screen_order || 1, status || 'active']
        );
        res.json({ status: true, message: 'Onboarding screen saved successfully!' });
    } catch (err) {
...
exports.update_onboarding = async (req, res) => {
    try {
        ...
        let query, params;
        if (req.file) {
            const image_url = '/uploads/onboarding/' + req.file.filename;
            query = `UPDATE onboarding_screens
                     SET title=?, description=?, image_url=?, screen_order=?, status=?
                     WHERE id=?`;
            params = [title, description, image_url, screen_order, status, id];
        } else {
            query = `UPDATE onboarding_screens
                     SET title=?, description=?, screen_order=?, status=?
                     WHERE id=?`;
            params = [title, description, screen_order, status, id];
        }
        await connection.query(query, params);
        res.json({ status: true, message: 'Onboarding screen updated successfully!' });
    } catch (err) {
...
exports.delete_onboarding = async (req, res) => {
    try {
        ...
        res.json({ status: true, message: 'Onboarding screen deleted successfully!' });
    } catch (err) {
...
exports.add_user = async (req, res) => {
    try {
        const {
            phone_number, full_name, email, date_of_birth,
            gender, location, is_profile_complete, terms_accepted
        } = req.body;
...
exports.block_user = async (req, res) => {
    try {
        ...
        res.json({ status: true, message: `User ${status === 'blocked' ? 'blocked' : 'unblocked'} successfully` });
    } catch (err) {
        console.error(err);
...
exports.update_giveaway_status = async (req, res) => {
    try {
        ...
        await connection.query('UPDATE giveaways SET status = ? WHERE id = ?', [status, id]);
